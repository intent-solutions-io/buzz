...
| 003 | [OD-DEPL — Deploy posture (public-safe)](003-OD-DEPL-deploy-posture-public.md) | Hosting shape, manual digest-pinned update lane, edge controls, and recovery gates. Operator detail lives in the separate private `intent-os/ops/buzz/` lane. |
...
| 008 | [AA-AUDR — Fork allowlist lockstep audit 2026-08-08](008-AA-AUDR-fork-allowlist-lockstep-2026-08-08.md) | The `ALLOW` array granted `.gitleaksignore`, a path `FORK.md` never declared, that does not exist and that no tool uses. Removed rather than documented, per `decision-log/048` D170/D171 — removal fails closed. No breach, no divergence change. |
| 009 | [AA-AUDR — Fork-contract breach audit 2026-08-16](009-AA-AUDR-fork-contract-breach-2026-08-16.md) | PRs #26/#27 carried the invite→default-channel patch on fork `main` (11 upstream-owned paths modified + carried e2e spec + migration backports; prod froze on fork image `intent-buzz:9ba98c220`). Process breach only. Remediated 2026-08-31: upstream sync, `CARRIED_PATCHES` emptied, prod back on the upstream published digest, enrollment moved to an ops-side watcher; real fix tracked as upstream #4307. |
